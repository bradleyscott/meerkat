...
If you can see a file path in the conversation context or the user's selection, use that. Otherwise, ask the user for the file path.
...
11. If `confluence_url` does NOT exist in the frontmatter:
    ...
    - After successful creation, update the original markdown file's frontmatter:
      ...
      - Add `confluence_url: <new-page-url>` to the YAML frontmatter
