---
description: Push markdown files to external source (Confluence, Notion, and more) (project)
...
3. **Resolve the source URL** using this priority order:
   ...
   c. If still no URL, ask: "No source URL found for {filename}. Paste the URL to sync with (Notion, Confluence, etc.):"
...
7. **Detect the source platform** using this priority order:
   a. Infer from the `source_url` domain:
      - `atlassian.net` → **Confluence**
      - `notion.so` → **Notion**
   b. Read `config.json` from the project root. If `source.default_platform` is set, use that value.
   ...
   d. Use `AskUserQuestion` to ask: "Which source platform? (Confluence / Notion)"
      - Save the answer to `config.json` under `source.default_platform` for future use
