...
1. **Replace links with Notion URLs**:
   ...
   **Relative markdown links** — scan for `[text](./path/to/file.md)` or `[text](../path/to/file.md)` or `[text](path/to/file.md)`:
   - For each relative link found:
     ...
     d. If a `source_url` exists, replace the relative link with the Notion URL
   ...
   **Obsidian wikilinks** — scan for `[[Page Name]]`, `[[Page Name|Display Text]]`, and `[[@Person Name]]`:
   - For each wikilink found:
     ...
     d. If a `source_url` exists in the frontmatter, replace the wikilink with `[display text](source_url)` (use display text if provided, otherwise the page name)
     e. If no matching file is found or no `source_url` exists, replace the wikilink with plain text (the display text or page name) so it renders cleanly in Notion rather than showing raw `[[brackets]]`
...
3. **Diff and confirm** (only when `source_url` EXISTS — i.e. updating an existing page):
   - Use `notion-fetch` to retrieve the current Notion page content using the `source_url`
...
5. If `source_url` does NOT exist in the frontmatter:
